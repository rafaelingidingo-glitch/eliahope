...
async function main() {
  ...
  const programRecords: { id: string }[] = []
  for (const p of programs) {
    const program = await prisma.program.create({ data: p })
    programRecords.push({ id: program.id })
  }
  ...
  // Campaigns
  const campaignRecords: { id: string }[] = []
  const campaigns = [
  ...
  for (const c of campaigns) {
    ...
    campaignRecords.push({ id: campaign.id })
  }
  ...
  // Donations — using Prisma enum values
  const donations = [
    { donorName: 'John Mwangi', donorEmail: 'john@example.com', amount: 500000, currency: 'TZS', method: 'mpesa' as const, type: 'one_time' as const, campaignId: campaignRecords[0].id, status: 'successful' as const, message: 'Keep up the great work!' },
    { donorName: 'Sarah Johnson', donorEmail: 'sarah@example.com', amount: 1000000, currency: 'TZS', method: 'mpesa' as const, type: 'one_time' as const, campaignId: campaignRecords[1].id, status: 'successful' as const, message: 'Happy to support!' },
    { donorName: 'David Kimaro', donorEmail: 'david@example.com', amount: 200000, currency: 'TZS', method: 'mpesa' as const, type: 'monthly' as const, campaignId: campaignRecords[2].id, status: 'successful' as const },
    { donorName: 'Emily Chen', donorEmail: 'emily@example.com', amount: 750000, currency: 'TZS', method: 'bank_transfer' as const, type: 'one_time' as const, campaignId: campaignRecords[0].id, status: 'pending' as const },
    { donorName: 'Michael Brown', donorEmail: 'michael@example.com', amount: 300000, currency: 'TZS', method: 'crdb' as const, type: 'monthly' as const, campaignId: campaignRecords[1].id, status: 'successful' as const },
    { donorName: 'Anna Mbeki', donorEmail: 'anna@example.com', amount: 150000, currency: 'TZS', method: 'mpesa' as const, type: 'one_time' as const, campaignId: campaignRecords[3].id, status: 'successful' as const },
    { donorName: 'Robert Wilson', donorEmail: 'robert@example.com', amount: 2000000, currency: 'TZS', method: 'bank_transfer' as const, type: 'one_time' as const, campaignId: campaignRecords[1].id, status: 'failed' as const },
    { donorName: 'Grace Joseph', donorEmail: 'grace@example.com', amount: 400000, currency: 'TZS', method: 'mpesa' as const, type: 'monthly' as const, campaignId: campaignRecords[2].id, status: 'successful' as const },
    { donorName: 'Peter Andrew', donorEmail: 'peter@example.com', amount: 600000, currency: 'TZS', method: 'crdb' as const, type: 'one_time' as const, campaignId: campaignRecords[0].id, status: 'pending' as const },
  ...
  // Volunteers — using Prisma enum values
  const volunteers = [
    { name: 'Amina Hassan', email: 'amina@example.com', phone: '+255 234 567 890', skills: 'Teaching,First Aid', motivation: 'I want to give back to my community and help children succeed.', status: 'approved' as const, programId: programRecords[0].id },
  ...
  // Children — using Prisma enum values
  const children = [
  ...
  const childRecords: { id: string }[] = []
  for (const c of children) {
    const child = await prisma.child.create({ data: c })
    childRecords.push(child)
  }

  // Sponsors — using Prisma enum values
  const sponsors = [
    { name: 'Maria Garcia', email: 'maria@example.com', phone: '+1 555 123 4567', childId: childRecords[0].id, amount: 150000, frequency: 'monthly' as const, status: 'active' as const, startDate: new Date('2024-01-15') },
    { name: 'Robert Chen', email: 'robert.c@example.com', phone: '+86 555 987 6543', childId: childRecords[2].id, amount: 200000, frequency: 'monthly' as const, status: 'active' as const, startDate: new Date('2024-03-01') },
    { name: 'Jennifer Williams', email: 'jen.w@example.com', childId: childRecords[5].id, amount: 150000, frequency: 'monthly' as const, status: 'active' as const, startDate: new Date('2024-06-10') },
  ...
  // Newsletter subscribers — using Prisma enum values
  const subscribers = [
