  ...
  const handleResendConfirmationEmail = async () => {
    ...
    try {
      const res = await fetch('/api/email/donation-confirm', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: (activeTab === 'mpesa' ? mpesaEmail : crdbEmail) ? 'resend' : 'send',
  ...
  return (
    <div className="min-h-screen bg-[#f5f3ef]">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <button
              onClick={() => router.push('/')}
              className="flex items-center gap-2 text-[#44474d] hover:text-[#031632] font-medium text-sm transition-colors"
            >
              <ArrowLeft className="h-4 w-4" />
              {t.adminDashboard.backToWebsite || 'Back to Website'}
            </button>
            <div className="flex items-center gap-3">
              <img src="/logo.jpeg" alt="Elia's Hope" className="h-8 w-8 rounded-full object-cover" />
              <span className="text-[#031632] font-bold text-sm hidden sm:inline">Elia&apos;s Hope</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        {/* Header Card */}
        <div className="bg-[#031632] px-6 py-5 rounded-[5px] mb-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#ff8928] rounded-[5px] flex items-center justify-center">
        ...
        <div className="bg-white p-6 shadow-lg rounded-b-[5px]">
          <AnimatePresence mode="wait">
            ...
            {isSuccess ? (
              ...
              >
                ...
                <div className="border-2 border-dashed border-[#c5c6ce] rounded-[5px] p-5 text-left max-w-xs mx-auto mb-6">
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 px-5 py-2.5 bg-[#031632] text-white rounded-[5px] font-semibold hover:bg-[#1a2b48] transition-colors text-sm"
                  >
                    <Download className="h-4 w-4" /> {t.donation.downloadReceipt}
                  </button>
                  <button
                    onClick={handleShare}
                    className="flex items-center justify-center gap-2 px-5 py-2.5 bg-[#ff8928] text-white rounded-[5px] font-semibold hover:bg-[#e07820] transition-colors text-sm"
                  >
                  ...
                  {(activeTab === 'mpesa' ? mpesaEmail : crdbEmail) && (
                    <button
                      ...
                      className="flex items-center justify-center gap-2 px-5 py-2.5 bg-white border-2 border-[#ff8928] text-[#ff8928] rounded-[5px] font-semibold hover:bg-[#ffdcc6]/20 transition-colors text-sm disabled:opacity-50"
                    >
                  ...
                  <button
                    onClick={handleClose}
                    className="flex items-center justify-center gap-2 px-5 py-2.5 border-2 border-[#c5c6ce] text-[#031632] rounded-[5px] font-semibold hover:bg-gray-50 transition-colors text-sm"
                  >
                    {t.donation.close}
                  </button>
                </div>
              ...
              >
                  ...
                  <div className="mb-5">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-4 py-3 border-2 border-[#c5c6ce] rounded-[5px] text-[#031632] text-sm focus:outline-none focus:border-[#ff8928] transition-colors appearance-none bg-white pr-10"
                      >
                    ...
                    {selectedCampaign && (() => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="mt-2 bg-[#f5f3ef] rounded-[5px] p-2.5"
                        >
                ...
                {/* Payment Tabs */}
                <div className="flex mb-5 border-2 border-[#c5c6ce] rounded-[5px] overflow-hidden">
                  <button
                ...
                {activeTab === 'mpesa' && (
                  <form onSubmit={handleMpesaSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-11 pr-10 py-3 border-2 border-[#c5c6ce] rounded-[5px] text-[#031632] text-sm focus:outline-none focus:border-[#ff8928] transition-colors"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#44474d] font-semibold text-sm">+255</span>
                        <input
                          type="tel"
                          value={mpesaPhone}
                          onChange={(e) => setMpesaPhone(e.target.value)}
                          className="w-full pl-16 pr-10 py-3 border-2 border-[#c5c6ce] rounded-[5px] text-[#031632] text-sm focus:outline-none focus:border-[#ff8928] transition-colors"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-14 pr-10 py-3 border-2 border-[#c5c6ce] rounded-[5px] text-[#031632] font-semibold text-lg focus:outline-none focus:border-[#ff8928] transition-colors"
                    ...
                    {donationState === 'error' && (
                      <motion.div
                        ...
                        className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-[5px]"
                      >
                        <AlertCircle className="h-4 w-4 text-red-500 mt-0.5 flex-shrink-0" />
                        <div>
                          <p className="text-red-700 text-sm font-medium">{errorMessage}</p>
                          <button onClick={handleRetry} className="text-red-600 text-xs font-semibold underline mt-1">
                            {t.donation.tryAgain}
                          </button>
                        </div>
                      </motion.div>
                    ...
                    <button
                      type="submit"
                      disabled={donationState === 'loading'}
                      className="w-full py-3.5 bg-green-600 text-white rounded-[5px] shadow-xl font-semibold text-base hover:bg-green-700 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                    >
                    ...
                    {donationState === 'loading' && (
                      ...
                      >
                        ...
                        <div>
                          <p className="text-green-700 text-sm font-medium">{t.donation.checkPhoneMpesa}</p>
                          <p className="text-green-600 text-xs mt-0.5">{t.donation.waitingConfirmation}</p>
                        </div>
                      </motion.div>
                    )}
                  </form>
                  ...
                  crdbStep === 'otp' ? (
                    <form onSubmit={handleCrdbOtpSubmit} className="space-y-4">
                      <div className="text-center py-2">
                        ...
                        <h3 className="text-lg font-bold text-[#031632] mb-1">{t.donation.otpTitle}</h3>
                        <p className="text-[#44474d] text-sm">{t.donation.otpDescription}</p>
                      </div>

                      <div className="bg-[#f5f3ef] rounded-[5px] p-3 space-y-1 text-sm">
                        <div className="flex justify-between">
                          <span className="text-[#44474d]">{t.donation.amount}</span>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-4 py-3.5 border-2 border-[#c5c6ce] rounded-[5px] text-[#031632] text-center font-mono text-2xl tracking-[0.5em] focus:outline-none focus:border-[#ff8928] transition-colors"
                          placeholder="------"
                          maxLength={6}
                          required
                      ...
                      <button
                        ...
                        className="w-full py-3.5 bg-[#031632] text-white rounded-[5px] shadow-xl font-semibold text-base hover:bg-[#1a2b48] active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                      >
                        {crdbOtpConfirming ? (
                          <>
                            <Loader2 className="h-5 w-5 animate-spin" />
                    ...
                    <form onSubmit={handleCrdbSubmit} className="space-y-4">
                      <div className="flex items-center gap-2 p-3 bg-[#031632]/5 rounded-[5px] mb-1">
                        <Shield className="h-4 w-4 text-[#031632]" />
                        <p className="text-[#031632] text-xs font-medium">{t.donation.securePayment}</p>
                      </div>
                      ...
                      {crdbDonationState === 'loading' && (
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          className="flex items-center gap-3 p-3 bg-[#031632]/5 border border-[#031632]/10 rounded-[5px]"
                        >
                          <Building2 className="h-5 w-5 text-[#031632] animate-pulse" />
                          <div>
                            <p className="text-[#031632] text-sm font-medium">{t.donation.connectingCrdb}</p>
                            <p className="text-[#44474d] text-xs mt-0.5">{t.donation.authorizeCrdb}</p>
                          </div>
                        </motion.div>
                      )}

                      <div className="flex items-center gap-2 p-2.5 bg-green-50 border border-green-200 rounded-[5px]">
                        <Shield className="h-3.5 w-3.5 text-green-600 flex-shrink-0" />
                        <p className="text-green-700 text-xs">{t.donation.bankDetailsEncrypted}</p>
                      </div>
                    </form>
                  )
                )}
                ...
                {campaigns.length > 0 && (
                  <div className="mt-6 space-y-3">
                    ...
                    {campaigns.map((campaign, i) => {
                      ...
                      return (
                        <div key={campaign.id} className="bg-[#f5f3ef] rounded-[5px] p-3.5">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <div className="bg-[#ffdcc6] p-1 rounded-[5px]">
                          ...
                          <button
                            ...
                            className="w-full mt-2 py-2 text-xs font-semibold border-2 border-[#ff8928] text-[#ff8928] rounded-[5px] hover:bg-[#ff8928] hover:text-white transition-colors"
                          >
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#f5f3ef] flex items-center justify-center">
        <div className="animate-spin h-8 w-8 border-4 border-[#ff8928] border-t-transparent rounded-full" />
      </div>
    }>
      <DonatePageContent />
    </Suspense>
